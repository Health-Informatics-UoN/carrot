...
`UploadQueue` which posts to the message queue called `scanreports`), another to run the NLP service
...
([see here for building and running the Carrot-Mapper Docker image](/mapper/dev_guide/quickstart) ).
