...
<Callout type="warning">
  **Note:** The sample `sample-dataDictionary.csv` file in the sample data package is not related to the `ScanReport.xlsx` in terms of data accuracy. It's provided as an example format only. For this tutorial, we'll upload the scan report without a data dictionary.
</Callout>
