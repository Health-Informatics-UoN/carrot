...
| `--person-file`   | `PERSON_FILE`        | CSV file where the first column contains person IDs. This is used as a registry for all valid person IDs. Person IDs not found in this registry will be omitted from all OMOP tables. This file can also be used as an input file if it has additional data and is located in the input directory. |
...
| `--input-db-url`         | `INPUT_DB_URL`       | None    | SQLAlchemy connection string for database input. This is an alternative to `--input-dir`, so it cannot be used with that parameter. The system still needs `--person-file` or `PERSON_FILE`, but only the name (sans extension) will be used to determine the table; i.e. `C:/my/files/patient_data.csv` will use the `patient_data` table. |
