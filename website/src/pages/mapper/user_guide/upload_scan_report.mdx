...
In order to create a new Scan Report in Carrot Mapper system,
...
<Callout type="info">
  If there is no appropriate _Dataset_ already existing, click the
    _New Dataset_ button to the right of the _Dataset_ dropdown. This opens
    a panel.
    ...
    as appropriate (see
    [Projects, Datasets, Scan Reports, and Permissions ](projects_datasets_scanreports)
    for an explanation of these. The _Dataset_ creator is automatically
    assigned as an editor.

    Finally, click the "Create Dataset" button
...
WhiteRabbit tool. Broadly, this is an Excel .xlsx file with the
...
2. The first sheet is 'Field Overview' where:
   ...
   - The first 10 column headers are:
     ...
     "N unique values",
     "Fraction unique"
   - A contiguous group of rows is associated to each table in the
     ...
     added.
   - Within each group, a single row corresponds to a single field
...
"Observation", "Frequency"` etc.
   - The fields in the sheet must match exactly with the field names
     provided for this table in the 'Field Overview' sheet.
   - Each pair of columns is populated by all values that appear in the
   ...
   - Pairs of columns associated to a given field can differ in length
     from pairs of columns associated to other fields.
   - An entry in a given pair of columns which is blank in both the field
     name column and Frequency column will count as the end of the column.
   - If all entries in a given field should be redacted, it can be helpful
