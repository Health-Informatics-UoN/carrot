...
For your convenience, the compose file is already configured with enough environment variables to run the Carrot Mapper. For the reference, please see the [Configuration section](/mapper/deployment/configuration).
...
- OMOP Lite: `omop-lite` - This service will load the vocabularies into the database depending on how you configured it in the last step.

After the services are up and running, you can access the Carrot Mapper frontend at `http://localhost:3000` and the backend at `http://localhost:8000`. Default admin credentials are `admin` and `admin`.
