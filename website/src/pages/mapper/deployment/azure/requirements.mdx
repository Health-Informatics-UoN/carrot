...
For Carrot, you would need to create 4 web apps: one for Backend (Django), one for Frontend (NextJS), and two for Airflow.
...
Carrot uses Azure storage to store `blobs` (e.g., Scan Reports and Data dictionaries files).
...
Carrot uses Key vault to store secrets and keys in Backend, Frontend and Airflow apps.
...
Carrot uses Azure monitors for important components, e.g., Backend, Frontend and Airflow apps.
