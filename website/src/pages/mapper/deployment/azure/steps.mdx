...
After that, Carrot will proceed to the next steps.

- **Automatic Data Seeding**: The Carrot will seed the app database automatically with the OMOP table and field names.

<div id="superuserCredentials">
  - **Super User Creation**: A **default Django super user** will be created
...
<div>
  Once all containers are running successfully and a superuser has been created
  (if required), the initial instances of Carrot must be set up after the
  backend application is up and running.
</div>

Please follow these steps to create the first instances:

- **To access the Carrot Admin portal**, use the given URL: `<backend-URL>/admin/`.
- **Log in** using the **default [superuser credentials](#superuserCredentials)** or the **new superuser credentials created in the previous step**.
- Then **create first** `Data partner`, `Dataset`, and `Project`.
