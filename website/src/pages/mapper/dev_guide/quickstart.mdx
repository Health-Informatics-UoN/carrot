...
<Callout type="info">
  Please note that all vocabularies are large files, so downloading them may take a while. You could **either download all of them** or just the **minimum required vocabularies**.
</Callout>

**The minimum required vocabularies** are the ones that are **generally required to run the Carrot Mapper app**. The rest of the vocabularies are optional and can be downloaded later if needed.
