...
- Vocabularies downloaded from [Athena](https://athena.ohdsi.org/vocabulary/list). Login to Athena requires an account which can be obtained for free.
...
Navigate to your Docker Desktop, choose `api` container, choose `Exec` tab, then run:
...
Try using Carrot by steps in the [user quickstart](/mapper/user_guide/quickstart) to see if Carrot is running correctly.
