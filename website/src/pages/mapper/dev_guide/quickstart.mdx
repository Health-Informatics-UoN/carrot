...
to start the application stack. This runs the database, Azurite emulator, Azure functions (`workers`) and will build Carrot's backend (`web`) and frontend (`next-client`). This process will be on the background and you can check it through Docker Desktop app. After the command run successfully, Carrot can be accessed through `http://localhost:3000/`
...
{/* TODO: Check again the below after deleting React code */}
