...
### Storage and Queue setup

You need to set up an local storage for local development of Carrot.


**Note:** The default storage is `MinIO` and the Queue is `Azure Blob Queue`. 
<div>
  <Callout type="info">
    If you wish to use `MinIO` as a Storage and `Azure Blob Queue` as a Queue, you can skip this step.
  </Callout>
</div>

However, If you wish to **use Azure Blob and Blob Queue as a Storage and Queue**, do the following steps:

1. Locate the `docker-compose.yml` file in the `root` directory of Carrot.

2. Locate the `api` and `workers` services in the `docker-compose.yml` file.

3. Change the environment variables of `STORAGE_TYPE` from `minio` to `azure` for both services (see example below).
...
4. Save the changes and your Storage and Queue setup is now ready to build. 
...
This runs the following containers below: 
...
Since we already setup the storage and queue configurations, carrot will automatically create the necessary resources for you [(see the configurations)](#storage-and-queue-setup).

The resources are:
<div>
  <Tabs items={['MinIO', 'Azure Blob']}>
    <Tabs.Tab label="MinIO">
      <div>
        <div>
          <strong>BUCKETS</strong> = [ `scan-reports`, `data-dictionaries`, `rules-exports` ]
        </div>
        <div>
          <strong>QUEUES</strong> = [ `rules-local`, `rules-exports-local`, `uploadreports-local` ]
        </div>
      </div>
    </Tabs.Tab>
    <Tabs.Tab label="Azure Blob">
      <div>
        <div>
          <strong>BLOB CONTAINERS</strong> = [ `scan-reports`, `data-dictionaries`, `rules-exports` ]
        </div>
        <div>
          <strong>QUEUES</strong> = [ `rules-local`, `rules-exports-local`, `uploadreports-local` ]
        </div>
...
**Automatic Containers & Queues Creation**

- If you are using `MinIO` as a storage, the carrot will create the necessary buckets and set up the Queues for you automatically.

- If you are using `Azure Blob` as a storage, the carrot will create the necessary Blob Containers and set up the Queues for you automatically.

---

**[Optional] - Checking the resources**

<div>
  You can check the created resources by following the steps below:    
</div>


- If you are **using `MinIO` as a storage**, do the following steps: 
...
- If you are **using `Azure Blob` as a storage**, do the following steps:

  1. Open `MS Azure Storage Explorer` app on your local machine.

  2. Locate `Storage Accounts` -> `Emulator` -> `Blob Containers` and you will see the created Blob Containers.

  3. Locate `Storage Accounts` -> `Emulator` -> `Queues` and you will see the created Queues.
