...
**There are two options for storage services: `minio` or `azure`.**

1. **Azure as a Storage service:** 

  If you want to use Azure as a Storage service, use the following command:

  ```bash
  docker compose up -d
  ```

2. **MinIO as a Storage service:** 

  If you want to use MinIO as a Storage service, use the following command:
...
### Resource Information & Validation [Optional]

Since we already decided the type of storage service and warmed up the `docker-compose`, carrot will automatically create the necessary resources for you (see the configurations).
...
1. If you are using `Azure Blob` as a storage:

```bash
docker compose up -d
```

2. If you are using `MinIO` as a storage:
