...
Carrot's Compose Container stack consists of 6 services or containers working together. This page will give details about these services' configurations.
...
This service runs a PostgreSQL database in the background for Carrot on port `5432:5432`.

### Azure local storage: `azurite`

This service runs a Azure local storage in the background for Carrot's workers on port `10000:10000`, `10000:10001` and `10000:10002`.

The `command` and `AZURITE_ACCOUNTS` environment variable in this service make sure the connection between `azurite` and `workers` proper.

### OMOP CDM Loading: `omop-lite`
...
When `omop` schema existed, this service will closes automatically.
