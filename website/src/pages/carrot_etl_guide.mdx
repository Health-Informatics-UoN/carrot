...
<div>
The following document sets the Standardised Opera.ng Procedure (SOP) that must be followed and applied during the engineering of data for conversion to an [OMOP](https://www.ohdsi.org/data-standardization/) representation. 
</div>

Figure 1 illustrates the 3 main data engineering stages and the associated soQware tools that are used, this process is commonly known as an [Extract, Transform and Load (ETL) process](https://ohdsi.github.io/TheBookOfOhdsi/ExtractTransformLoad.html). 
...
<figure style={{ textAlign: "center" }}>
  ...
  <figcaption style={{ marginTop: "8px", fontStyle: "italic", color: "#666" }}>
    Figure 1: The stages of Health Infomatics Data Engineering.
  </figcaption>
</figure>
...
<div>
The pre-processing stage focuses on the ‘important aXributes’ of a dataset that need to be mapped to the OMOP Common Data Model for discoverability.
...
The output of Stage 1 is an anonymised dataset ready for profiling together with a formaXed data dictionary.
</div>
...
The pre-processed dataset is loaded into the OHDSI White Rabbit tool, this produces a sta.s.cal profile of the data called a ‘Scan Report’ that further de-identifies data by removing the rela.onships between data attributes. This is then checked to ensure that the data has been rigorously anonymised.
...
<div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "10px" }}>
  ...
  <div style={{ flex: "1" }}>
    ...
    - Define communica.on and feedback mechanisms and frequency thereof – these can be emails, weekly meetings, etc.
    </div>
...
<div id="pii-section">
  ...
  PII refers to data that can be used to identify, locate, or contact individuals or establishments, or reveal the characteristics or other details about them. PII might consist of direct iden.fiers, such as the name, social security number or other information that is unique to an individual. Indirect identifiers include uncommon race, ethnicity, extreme age, unusual occupa.on and other details [CDC](https://www.cdc.gov/nchs/training/confidentiality/training/page581.html).
...
<div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "10px" }}>
  ...
  - Create a demographics table, consisting of:
      ...
      - Ethnicity
  <div>
  - The rest of the data can then be arranged as deemed fit by the data owner and / or the CARROT data team, each table must include:
  ...
  <div style={{ marginTop: "0px" }}>
  - First, check the [CARROT4OMOP Data Standards](https://carrot4omop.ac.uk/standards/#data-dictionary-file) page for exact requirements.
...
<div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "10px" }}>
  ...
  - Check that there is a date event to map to per dataset table and that it conforms to [CARROT4OMOP](https://carrot4omop.ac.uk/standards/#data-dictionary-file).
...
<div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "20px" }}>
  ...
  **Upload Scan Report**
  ...
  - Log into [CARROT](https://ccom.azurewebsites.net/):
  - Upload Scan Report:
